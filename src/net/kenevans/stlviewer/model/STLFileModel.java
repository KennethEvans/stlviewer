...
public class STLFileModel implements IConstants {
	...
	public STLFileModel(String fileName) {
		...
		oruxMapsBpmFormatter = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'", Locale.US);
			...
			for (TrksegType trackSegment : trackSegments) {
				...
				for (WptType tpt : trackPoints) {
					...
					if (prevTime != -1) {
						...
						deltaLength = GpxUtils.greatCircleDistance(prevLat, prevLon, lat, lon);
	 ...
	 * @return
	 */
	boolean getHrFromOruxMapsBpm(String bpm, ArrayList<Double> hrValsArray, ArrayList<Long> hrTimeValsArray) {
		boolean res = true;
		double hr;
		if (bpm == null || bpm.length() == 0) {
			return false;
		...
		for (TrkType track : tracks) {
			...
			for (TrksegType trackSegment : trackSegments) {
				...
				for (WptType waypoint : waypoints) {
					System.out.println("(" + waypoint.getLat() + "," + waypoint.getLon() + "," + waypoint.getEle() + ")"
							+ " " + waypoint.getTime());
	...
	public static String sysInfo() {
		String info = "";
		String[] properties = { "user.dir", "java.version", "java.home", "java.vm.version", "java.vm.vendor",
				"java.ext.dirs" };
		String property;
		for (int i = 0; i < properties.length; i++) {
			property = properties[i];
			info += property + ": " + System.getProperty(property, "<not found>") + LS;
		}
		info += getClassPath("  ");
		return info;
	}
	...
	public String getInfo() {
		...
		int durationSec = (int) (duration / 1000.) - durationHours * 3600 - durationMin * 60;
		int hrDurationHours = (int) (duration / 3600000.);
		int hrDurationMin = (int) (hrDuration / 60000.) - hrDurationHours * 60;
		int hrDurationSec = (int) (hrDuration / 1000.) - hrDurationHours * 3600 - hrDurationMin * 60;
		info += "Tracks: " + startDate + " to " + endDate + LS;
		info += String.format("Duration: %d hr %d min %d sec (%.0f sec)", durationHours, durationMin, durationSec,
				duration / 1000.) + LS;
		...
		if (nHrValues != 0) {
			...
			info += String.format("HR Duration: %d hr %d min %d sec", hrDurationHours, hrDurationMin, hrDurationSec)
					+ LS;
			stats = getTimeAverageStats(hrVals, hrTimeVals, -Double.MIN_VALUE);
			if (stats != null) {
				info += String.format("HR Min=%.0f HR Max=%.0f HR Avg=%.1f", stats[0], stats[1], stats[2]) + LS;
			} else {
				// Get simple average
				stats = getSimpleStats(hrVals, hrTimeVals, -Double.MIN_VALUE);
				if (stats != null) {
					info += String.format("HR Min=%.0f HR Max=%.0f HR Avg=%.1f" + " (Simple Average)", stats[0],
							stats[1], stats[2]) + LS;
				}
			}
		}
		if (speedVals.length != 0) {
			...
			if (stats != null) {
				info += String.format("Speed Min=%.1f Speed Max=%.1f Speed Avg=%.1f mi/hr", stats[0], stats[1],
						stats[2]) + LS;
			} else {
				// Get simple average
				stats = getSimpleStats(speedVals, speedTimeVals, -Double.MIN_VALUE);
				if (stats != null) {
					info += String.format("Speed Min=%.1f Speed Max=%.1f Speed Avg=%.1f mi/hr" + " (Simple Average)",
			...
			if (stats != null) {
				...
				if (stats != null) {
					info += String.format("  Moving Speed Avg=%.1f mi/hr" + " (Simple Average)", stats[2]) + LS;
				}
			}
		...
		if (eleVals.length != 0) {
			...
			if (stats != null) {
				info += String.format("Ele Min=%.0f Ele Max=%.0f Ele Avg=%.0f ", stats[0], stats[1], stats[2]);
			}
			if (stats1 != null) {
				info += String.format("Ele Gain=%.0f Ele Loss=%.0f ft", stats1[0], stats1[1]) + LS;
			} else {
				info += LS;
			}
		} else {
			...
			if (stats != null) {
				info += String.format("Ele Min=%.0f Ele Max=%.0f Ele Avg=%.1f ft" + " (Simple Average)", stats[0],
						stats[1], stats[2]) + LS;
			}
		}
		info += nTracks + " Tracks" + "        " + nSegments + " Segments:" + LS;
		info += nTrackPoints + " Track Points" + "        " + nHrValues + " HR Values:" + LS;
		return info;
	}
	 ...
	 */
	public static double[] getSimpleStats(double[] vals, long[] timeVals, double omitBelow) {
		// System.out.println("vals: " + vals.length + ", timeVals: "
		// + timeVals.length);
		if (vals.length != timeVals.length) {
			Utils.errMsg("getSimpleStats: Array sizes (vals: " + vals.length + ", timeVals: " + timeVals.length
					+ ") do not match");
			return null;
		}
		int len = vals.length;
	 ...
	 * @return {min, max, avg} or null on error.
	 */
	public static double[] getTimeAverageStats(double[] vals, long[] timeVals, double omitBelow) {
		// System.out.println("vals: " + vals.length + ", timeVals: "
		// + timeVals.length);
		if (vals.length != timeVals.length) {
			Utils.errMsg("getTimeAverageStats: Array sizes (vals: " + vals.length + ", timeVals: " + timeVals.length
					+ ") do not match");
			return null;
		}
		int len = vals.length;
